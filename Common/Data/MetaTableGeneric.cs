...
namespace MaTech.Common.Data {
    public partial class MetaTableGeneric<TEnum> where TEnum : unmanaged, Enum, IConvertible {
        ...
        public bool Has<T>(string keyName) => Has<T>(new EnumEx<TEnum>(keyName, ordered: false)); 
        public bool Remove<T>(string keyName) => Remove<T>(new EnumEx<TEnum>(keyName, ordered: false)); 
        public T? Get<T>(string keyName) => Get<T>(new EnumEx<TEnum>(keyName, ordered: false)); 
        public T? GetNullable<T>(string keyName) where T : struct => GetNullable<T>(new EnumEx<TEnum>(keyName, ordered: false)); 
        public T GetOrSet<T>(string keyName, in T value) => GetOrSet<T>(new EnumEx<TEnum>(keyName, ordered: false), value); 
        public T Set<T>(string keyName, in T value) => Set<T>(new EnumEx<TEnum>(keyName, ordered: false), value); 
        ...
        public Selector<Root, TEnum> Select(string keyName) => CreateRootSelector(new EnumEx<TEnum>(keyName, ordered: false).Value);
        ...
        public partial struct Selector<TContext, TEnum0> {
            ...
            public Selector<Selector<TContext, TEnum0>, TEnum1> Select<TEnum1>(string keyName1) where TEnum1 : unmanaged, Enum, IConvertible => CreateNestedSelector(this, new EnumEx<TEnum1>(keyName1, ordered: false).Value);
        }
    }
}