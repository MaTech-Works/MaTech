...
namespace MaTech.Gameplay.Display {
    public class NoteLayer : MonoBehaviour {
        // todo: remove ratio calculate methods and give out Range<RollValue>
        // todo: 支持为每个VisualUnit实例化图形，而非为每个NoteCarrier实例化图形
        
        #region Private Fields - Runtime Containers
        
        private readonly struct VisualInfo {
            public readonly NoteCarrier carrier;
            public readonly GameObject obj;
            public readonly INoteVisual visual;
            public VisualInfo(NoteCarrier carrier, GameObject obj) {
                this.carrier = carrier;
                this.obj = obj;
                this.visual = obj.GetComponent<INoteVisual>();
            }
        }
        
        private PointerList<NoteCarrier> listCarrierEarlyRoll = null!;    // 按远端卷轴进入顺序排序
        private PointerList<NoteCarrier> listCarrierLateRoll = null!;     // 按近端卷轴退出顺序排序
        private PointerList<NoteCarrier> listCarrierEarlyTime = null!;    // 按判定时间进入顺序排序
        
        protected Dictionary<Carrier, INoteVisual>.KeyCollection RealizedCarriers => hashsetCarrierRealized.Keys;
        ...
        private class PrefabPool {
            ...
            private readonly NoteLayer self;
            ...
            public PrefabPool(NoteLayer self, PrefabEntry prefabEntry) {
        ...
        private double lastRoll = double.NegativeInfinity;
        ...
        private Func<NoteCarrier, bool> cachedCarrierSelectCondition = null!;
        private Func<NoteCarrier, bool>[] cachedIterateStopConditions = null!;
        private Action<NoteCarrier> cachedRealizationAction = null!;
        private Func<VisualInfo, bool> cachedVirtualizationAction = null!;
        private bool isActionAndFuncCached = false;

        private readonly Dictionary<Carrier, INoteVisual> hashsetCarrierRealized = new();
        private readonly List<VisualInfo> listObjectRealized = new();
        
        #endregion

        #region Public Methods - Setters and Getters
        
        internal bool IsPossiblyBarLayer => prefabEntries.Any(entry => entry.prefab?.IsBarObject() ?? false);
        ...
        // todo: cached per-keyword roll sampling
        public float DeltaRollToRatio(double roll) => (float)(roll / windowDeltaRollEarly);
        
        public float CalculateRatio(double roll, NoteCarrier carrier, bool clampToDisplayWindow) => DeltaRollToRatio(CalculateDeltaRoll(roll, carrier, clampToDisplayWindow));
        public double CalculateDeltaRoll(double roll, NoteCarrier carrier, bool clampToDisplayWindow) {
        ...
        public (double roll, float ratio) CalculateDeltaRollAndRatio(double roll, NoteCarrier carrier, bool clampToDisplayWindow) {
            var deltaRoll = CalculateDeltaRoll(roll, carrier, clampToDisplayWindow);
            return (deltaRoll, DeltaRollToRatio(deltaRoll));
        }

        public INoteVisual? FindVisual(NoteCarrier carrier) => hashsetCarrierRealized.GetOrNull(carrier);
        public T? FindVisual<T>(NoteCarrier carrier) where T : class => FindVisual(carrier) as T;
        ...
        public async UniTask Load(IList<NoteCarrier> carriers) {
            ...
            if (!isActionAndFuncCached) {
                ...
                cachedIterateStopConditions = new Func<NoteCarrier, bool>[] {
            ...
            listCarrierEarlyRoll = new PointerList<NoteCarrier>(selectedCarriers);
            listCarrierLateRoll = new PointerList<NoteCarrier>(selectedCarriers);
            listCarrierEarlyTime = new PointerList<NoteCarrier>(selectedCarriers);
        ...
        public int HandleNoteHit(IJudgeUnit unit, JudgeLogicBase.NoteHitAction action, in TimeValue time, HitResult result) {
            int count = 0;
            foreach (var carrier in RealizedCarriers) {
                if (carrier is not NoteCarrier noteCarrier) continue;
                if (!noteCarrier.ContainsUnit(unit)) continue;
                var visual = FindVisual<INoteHitEvent>(noteCarrier);
        ...
        private void SortCarriers() {
            ...
            ProcessorBasic.SortCarriersByRoll(listCarrierEarlyRoll, true, WindowDeltaRollScaledEarly);
            ProcessorBasic.SortCarriersByRoll(listCarrierLateRoll, false, WindowDeltaRollScaledLate);
            ShellSort.Hibbard(listCarrierEarlyTime, Carrier.ComparerStartTime());
        }
        
        private void IterateForward(PointerList<NoteCarrier> list, Func<NoteCarrier, bool> stopCondition, Action<NoteCarrier> actionOnForward) {
        ...
        private void IterateBackward(PointerList<NoteCarrier> list, Func<NoteCarrier, bool> stopCondition, Action<NoteCarrier> actionOnBackward) {
            while (list.HasLast) {
        ...
        private bool IsCarrierRealized(NoteCarrier carrier) => hashsetCarrierRealized.ContainsKey(carrier);

        private void RealizeCarrierIfInRange(NoteCarrier carrier) {
            if (IsCarrierInRange(carrier) && !IsCarrierRealized(carrier))
                RealizeObject(carrier);
        }

        private void RealizeObject(NoteCarrier carrier) {
            ...
            var tuple = new VisualInfo(carrier, obj);
            tuple.visual.StartVisual(carrier, (NoteLayer)this);
        ...
        private void VirtualizeObject(VisualInfo tuple) {
        ...
        private bool IsCarrierInRange(NoteCarrier carrier) {
        ...
        void OnValidate() {
            for (int i = 0; i < prefabEntries.Length; i++) {
                ...
                if (prefabEntries[i].prefab!.GetComponent<INoteVisual>() == null) {
                    Debug.LogError($"<b>[{typeof(NoteLayer).Name}]</b> The object prefab {i} does not contain an IDisplayObject<{typeof(NoteCarrier).Name}, {typeof(NoteLayer).Name}> component on the root object", this);
